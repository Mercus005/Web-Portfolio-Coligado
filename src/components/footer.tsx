    ...
    <footer className="bg-ink-950 border-t border-ink-700">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 py-8">
          <Typography className="font-display text-paper font-medium text-sm sm:text-base order-2 sm:order-1">
            © {CURRENT_YEAR} Cliff Marvic M. Coligado
          </Typography>

          <ul className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 order-1 sm:order-2">
            {QUICK_LINKS.map((link) => (
              <li key={link.name}>
                <a
                  href={link.href}
                  className="text-sm text-paper-muted hover:text-aqua transition-colors"
                >
                  {link.name}
                </a>
              </li>
          ...
          <div className="flex items-center gap-3 order-3">
            {SOCIAL_LINKS.map((link) => (
              <a
                ...
                className="flex h-8 w-8 items-center justify-center rounded-full border border-ink-700 text-paper-muted hover:border-aqua hover:text-aqua transition-colors"
              >
                {link.icon}
              </a>
            ))}
